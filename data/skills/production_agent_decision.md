...
阶段4完成后、派发阶段5之前，根据模型参数 `多参` 和 `用户强制模式` 决定写入模式：

| 条件 | 决策层操作 |
|------|-----------|
| `用户强制模式` 为 `首位帧模式` | 以 **"首位帧模式"** 派发给执行层 |
| `用户强制模式` 为 `多参模式` | 以 **"多参模式（带提示词）"** 派发给执行层（生成 prompt+分镜图，但 track 按分镜表组分批） |
| `用户强制模式` 为 `无` 且 `多参` 为 `是` | 使用 **"纯文本多参模式"** 派发给执行层 |
| `用户强制模式` 为 `无` 且 `多参` 为 `否` | 无需询问用户，直接以 **"首位帧模式"** 派发给执行层 |
