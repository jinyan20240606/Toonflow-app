...
**videoDesc 位置声明要求**（与 prompt 一致）：
- 每个可见角色在 videoDesc 中首次出现时，必须显式声明其**场景内绝对位置**
- 多角色同框时须标注**互动距离关系**
- 机位关系须显式标注（如"平视机位""低角度仰拍""过@图N 肩膀"）
- 位置信息从分镜表「空间关系」「朝向」独立列及画面描述中提取，**不得遗漏**
- 视频生成模型同样不会推理上下文，每个分镜的 videoDesc 必须自包含
...
7. 每个可见角色在 prompt 中已显式声明空间位置
8. 多角色同框时已标注**互动距离关系**（如"面对面相距约一臂""隔中控台"）
9. 机位关系已显式标注（如"平视机位""低角度仰拍"）
