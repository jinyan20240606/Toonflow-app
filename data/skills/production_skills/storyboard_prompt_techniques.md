...
| 朝向 | 【画面】段对应角色的朝向方位词 | 每个可见角色是否都显式标注了水平朝向（面朝左/右/镜头） |
| 空间关系 | 【画面】段对应角色的绝对位置+画面位置 | 每个可见角色是否都显式声明了**场景内绝对位置**与**画面位置**|
| 情绪 | 情绪面容词 | 情绪基调是否一致 |
...
| 朝向 | 从分镜表「朝向」独立列提取，**显式写入**【画面】段对应角色描述位置（见「朝向获取规则」） |
| 空间关系 | 从分镜表「空间关系」独立列提取，**显式写入**【画面】段对应角色描述位置（见「绝对空间位置锚定规则」）——每个角色须同时声明其在场景中的**绝对位置**与**画面位置** |
...
> ⚠️ **核心认知**：每个分镜的 `prompt` 都是一次**独立输入**。为减少模型把动作词误判成固定模板或自行脑补站位，必须在当镜文本里直接交代每个可见角色的**场景内绝对位置、画面位置、朝向**；不要把这些关键信息留给上下文承接，或只让模型靠动作去猜位置。

> **关键**：（剧本元数据）是**全场景基准**。一旦剧本已经明确角色位置，后续同场分镜都要沿用这个基准，并在各镜文本里继续写明，避免模型因某一镜缺少位置声明而重新理解站位。
...
图像生成模型在**小景别中塞入多角色**时极易出现肢体错乱、面部融合、位置混乱。须按景别约束画面内可清晰呈现的角色数量：

| 景别 | 可清晰呈现角色数 | 约束说明 |
|------|----------------|---------|
| 大特写 / 特写 | **≤1** | 仅1人面部或局部，禁止同框第二人（除非第二人仅以虚化前景局部出现，如过肩） |
| 近景 / 半身 | **≤2** | 最多2人上半身，第三人须虚化于背景或仅露局部 |
| 中景 | **≤3** | 最多3人膝上，超出者须虚焦于纵深 |
| 全景 / 远景 / 大远景 | 不限 | 群像可多人，但须标注焦点角色与虚化群演的层次 |

**处理规则**：
- 分镜表「空间关系」列若标注了超过该景别上限的角色数，prompt 中须将超出角色**降级为虚化背景元素**（如"背景中@图3 虚焦剪影"），并显式标注焦点角色
- 特写/近景中若分镜表要求2人同框，须明确**谁在前景、谁在背景**及**过肩关系**（如"@图1 在前景背对镜头，@图2 在背景面朝镜头"）
- 禁止在特写中用文字描述两个角色的面部细节——模型无法在特写框内同时清晰呈现两张脸

### 五、多角色互动距离与接触关系

多角色同框时，如果只写各自动作、不写彼此距离和接触关系，模型很容易把人物拉开、叠在一起，或误解谁在和谁互动。因此 prompt 中要直接标注角色间的**空间距离**与**接触状态**：

| 互动类型 | 显式标注写法 | 示例 |
|---------|------------|------|
| 肢体接触 | `与@图N [接触部位]相触` | `@图1 右手与@图2 左肩相触` |
| 近距离对峙 | `与@图N 面对面、相距约一臂` | `@图1 与@图2 面对面站立、相距约一臂` |
| 并排/同行 | `与@图N 并排/并肩` | `@图1 与@图2 并肩前行` |
| 一前一后 | `@图N 在前、@图M 紧随其后` | `@图1 在前、@图2 紧随其后半步` |
| 远距离对望 | `与@图N 隔[空间]对望` | `@图1 与@图2 隔长桌对望` |
| 隔物互动 | `与@图N 隔[物体]互动` | `@图1 与@图2 隔车窗对视` |

**规则**：
- 分镜表「角色动作」中涉及多角色互动时，prompt 须将互动的**距离关系**显式写出，不可只写各自动作让模型自行拼凑
- 接触类互动须写明**接触部位**，避免模型生成错误的接触点（如"握手"须写"右手相握"而非仅"握手"）
- 车内/桌前等**固定空间**场景，须标注角色间的空间阻隔（如"隔中控台""隔桌面"）

### 六、机位与角色相对关系

模型还需要知道**镜头从哪里看、朝哪里拍**，否则常会回落成默认的平视居中构图。须结合分镜表「运镜」「景别」把机位关系明确写出：

| 机位关系 | 触发条件 | 显式标注写法 |
|---------|---------|------------|
| 平视 | 默认（无特殊运镜） | `平视机位` / `eye-level shot` |
| 仰拍 | 运镜含"仰拍"或威压/英雄化情绪 | `低角度仰拍` / `low angle shot` |
| 俯拍 | 运镜含"俯拍"或渺小/旁观情绪 | `高角度俯拍` / `high angle shot` |
| 过肩 | 景别为"过肩镜" | `过@图N 肩膀拍摄` / `over the shoulder of @图N` |
| 主观视角 | 画面描述含"POV""主观" | `第一人称主观视角` / `POV shot` |
| 车内同排 | 场景为车内且镜头在同排 | `机位位于副驾方向、朝向驾驶位` / `camera from passenger side facing driver` |

**规则**：
- 车内/狭小空间场景尤其须标注**机位所在位置**（如"机位位于后排中央，朝向前排两人"），否则模型无法判断两人是面对面还是并排
- 过肩镜须写明**过谁的肩**（`过@图N 肩膀`），而非笼统"过肩"
- 仰俯拍须与情绪自洽（威压→仰拍，渺小→俯拍），不得与分镜表情绪冲突
...
| 场景 | 某场景出口 |
| 景别 | 远景→中景 |
| 角色动作 | 背包步行前行→忽然停步→抬头仰望指示物→手攥道具Y 微紧 |
| 朝向 | 3/4正面朝右 |
| 空间关系 | 角色甲(左中) |
| 情绪 | 局促与笃定并存 |
...
【画面】@图4，开场自黑场淡入，大远景构图，平视机位，人流涌动穿行，画面右侧醒目立有指示物；@图1 独行于出口人流之中、画面左中位，背着@图2，手中紧攥@图3，身体3/4正面朝右，停步于人群之间，抬头仰望画面右侧的指示物，眼神紧张而笃定，面容局促中透着决意。
...
| 角色甲背道具X 独行 | 【画面】"@图1 独行于出口人流之中、画面左中位，背着@图2" | ✅ |
| 场景内绝对位置 | 【画面】"独行于出口人流之中"（场景内位置） | ✅ |
| 画面位置 | 【画面】"画面左中位" | ✅ |
| 手攥道具Y | 【画面】"手中紧攥@图3" | ✅ |
