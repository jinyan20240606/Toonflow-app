...
export default router.post(
  ...
  async (req, res) => {
    ...
    for (const { videoId, videoPath, prompt, duration, images } of tasks) {
      ...
      console.log(`[batchGenerateVideo] 开始生成视频, videoId=${videoId}, model=${model}`);
      aiVideo
        .run(
          {
            prompt,
        ...
        .then(async () => {
          console.log(`[batchGenerateVideo] 视频生成完成, videoId=${videoId}, 开始保存文件`);
          await aiVideo.save(videoPath);
        })
        .then(async () => {
          console.log(`[batchGenerateVideo] 视频文件保存成功, videoId=${videoId}, path=${videoPath}`);
          await u.db("o_video").where("id", videoId).update({ state: "生成成功" });
          console.log(`[batchGenerateVideo] 视频状态已更新为「生成成功」, videoId=${videoId}`);
        })
        .catch(async (error: any) => {
          console.error(`[batchGenerateVideo] 视频生成失败, videoId=${videoId}, error=`, u.error(error).message);
          await u
            .db("o_video")
            .where("id", videoId)
            .update({
